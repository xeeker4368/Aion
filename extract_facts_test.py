"""
Aion Fact Extraction Test

One-step extraction: conversation → facts
No summaries. No intermediate copies.
Each fact points back to its source conversation_id.

This is a test to compare single-step extraction quality
against the two-step consolidation (summary + facts).

Usage: python extract_facts_test.py
"""
...
def get_conversations():
    """Get all ended conversations with their messages."""
    conn = sqlite3.connect(str(WORKING_DB))
    conn.row_factory = sqlite3.Row

    conversations = conn.execute(
        "SELECT id, started_at FROM conversations WHERE ended_at IS NOT NULL ORDER BY started_at"
    ).fetchall()
...
def extract_facts(conversation):
    """Extract facts from a single conversation."""
    transcript_lines = []
    ...
    try:
        ...
        # Attach conversation_id to each fact
...
def main():
    ...
    logger.info(f"\nTotal: {len(all_facts)} facts from {len(conversations)} conversations")

    # Save to a JSON file for review
    output_path = Path("data/extracted_facts_test.json")
