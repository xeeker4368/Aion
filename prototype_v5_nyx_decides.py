"""
Prototype v5: Let Nyx Decide

Instead of pattern matchers, separate reasoning calls, or two-pass
systems — ask Nyx directly: "Do you need your memories for this?"

Nyx gets: conversation history + self-knowledge + SOUL.md
Nyx decides: search memory, use a tool, or just respond

If Nyx asks for memories, we search (hybrid BM25 + vector),
add the results, and let Nyx respond.

This tests whether the entity can reliably distinguish:
- Greetings (just respond)
- Questions about shared experiences (search memory)
- General knowledge questions (respond from training)
- Questions that could be either (search to be safe)
- Requests for current info (web search tool)

READ-ONLY. Nothing modified.

Run on Hades:
    cd ~/aion
    ./aion/bin/python prototype_v5_nyx_decides.py
"""
...
# ============================================================
# BM25 index (in-memory, read-only)
# ============================================================
...
def hybrid_search(collection, bm25, chunk_list, query,
...
NYX_DECISION_PROMPT = """You have a new message to respond to. Before you respond, decide what you need.
...
def nyx_decides(conversation_messages, user_message, self_knowledge, soul):
    """Ask Nyx what it needs to respond to this message."""
    # Build system prompt: conversation + self-knowledge + SOUL.md at end
    parts = []
...
def parse_decision(output):
    """Parse Nyx's decision into an action."""
    text = output.strip()
    upper = text.upper()
    ...
    if "SEARCH_MEMORY" in upper:
        # Extract the search terms after SEARCH_MEMORY
        idx = upper.find("SEARCH_MEMORY")
        query = text[idx + len("SEARCH_MEMORY"):].strip()
        # Clean up common artifacts
        query = query.lstrip(":").strip()
        if not query:
            query = text  # fallback: use the whole output
        return "memory", query

    # If it doesn't match any format, check for keywords
    lower = text.lower()
    if any(w in lower for w in ["search memory", "search my memory",
                                 "look back", "recall", "remember"]):
        return "memory", text

    # Default: treat as memory search if it's not clearly a greeting response
...
def run_test(collection, bm25, chunk_list, test_name, conv_messages,
    ...
    # --- Let Nyx decide ---
    print(f"\n--- NYX DECIDES ---")
    raw_decision = nyx_decides(conv_messages, user_message, self_knowledge, soul)
    print(f"  Raw output: \"{raw_decision}\"")

    action, search_query = parse_decision(raw_decision)
    print(f"  Action: {action}" + (f" | Query: \"{search_query}\"" if search_query else ""))

    if action == "direct":
        print(f"\n  Nyx chose to respond directly (no retrieval needed)")
        return

    if action == "web":
        print(f"\n  Nyx chose web search (tool call)")
        return

    if action == "memory":
        # --- Hybrid search with Nyx's query ---
        print(f"\n--- HYBRID SEARCH (Nyx's query) ---")
        ...
        # --- Also show hybrid with raw user message for comparison ---
        ...
        print(f"\n--- VERDICT ---")
        print(f"  Hybrid (Nyx query):    {'FOUND' if has_relay else 'MISSED'}")
...
def main():
    print("=" * 70)
    print("Prototype v5: Let Nyx Decide")
    print("=" * 70)
    ...
    print(f"SOUL.md: ~{len(soul)//4}t | Self-knowledge: ~{len(self_knowledge)//4}t")
    ...
    # Test 2: General knowledge (should respond directly from training)
    run_test(collection, bm25, chunk_list,
             "General knowledge (training data)",
             [], "What is quantum computing?", self_knowledge, soul)

    # Test 3: The Claude difference question (needs memory)
    run_test(collection, bm25, chunk_list,
             "Claude difference (needs memory)",
             all_messages[:4], all_messages[4]["content"],
             self_knowledge, soul)

    # Test 4: Summarize Claude (needs memory)
    run_test(collection, bm25, chunk_list,
    ...
    # Test 6: Ambiguous — could be training or memory
    run_test(collection, bm25, chunk_list,
             "Ambiguous — AI consciousness (training or memory?)",
             [], "What are your thoughts on AI consciousness?",
             self_knowledge, soul)

    # Test 7: Explicit memory reference
    run_test(collection, bm25, chunk_list,
             "Explicit memory — name choice",
             [], "Do you remember how you chose your name?",
             self_knowledge, soul)

    # Test 8: Something Nyx should know from experience
    run_test(collection, bm25, chunk_list,
             "Experience question — what has Nyx learned",
             [], "What's the most important thing you've learned so far?",
             self_knowledge, soul)

    print(f"\n{'=' * 70}")
    print("PROTOTYPE v5 COMPLETE")
    print(f"{'=' * 70}")
    conn.close()
